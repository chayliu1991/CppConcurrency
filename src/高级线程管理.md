...
可以在死循环中使用 std::this_thread::yield](https://en.cppreference.com/w/cpp/thread/yield) 来转让时间片，但这样做的问题是，如果线程池处于空闲状态，就会无限转让时间片，导致 CPU 使用率达100%。
...
对于其他阻塞调用（比如 `mutex`、`future`）的中断一般也可以像对 [std::condition_variable](https://en.cppreference.com/w/cpp/thread/condition_variable) 一样使用 `timeout`，因为不访问内部 `mutex` 或 `future` 无法在未满足等待的条件时中断等待。
